...
-- (for a test)
-- sum(round(sum(spend_value)/count(distinct session_id),2)) over (order by round(sum(spend_value)/count(distinct session_id),2) desc rows between unbounded preceding and current row) as cumulative_avg_spend 
from sessions_info
...
-- (for a test)
-- sum(sum(spend_value)/count(distinct session_id)) over (order by sum(spend_value)/count(distinct session_id) desc rows between unbounded preceding and current row) as cumulative_avg_spend
from sessions_info
where session_id is not null
group by 1
order by 3 desc;
